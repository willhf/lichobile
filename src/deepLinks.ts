...
export default {
  init() {
    ...
    universalLinks.subscribe(
      'challenge',
      (eventData: UniversalLinks.EventData) =>
        router.set('/challenge/' + eventData.path.split('/').pop())
    )
    ...
    universalLinks.subscribe(
      'tournamentDetail',
      (eventData: UniversalLinks.EventData) =>
        router.set('/tournament/' + eventData.path.split('/').pop())
    )
    ...
    universalLinks.subscribe(
      'tvChannel',
      (eventData: UniversalLinks.EventData) =>
        router.set('/tv/' + eventData.path.split('/').pop())
    )
    universalLinks.subscribe('userVariantProfile', handleVariantProfile)
    universalLinks.subscribe('userTV', (eventData: UniversalLinks.EventData) =>
      router.set('/@/' + eventData.path.split('/')[2] + '/tv')
    )
    universalLinks.subscribe(
      'userProfile',
      (eventData: UniversalLinks.EventData) =>
        router.set('/@/' + eventData.path.split('/').pop())
    )
    universalLinks.subscribe(
      'signupConfirm',
      (data: UniversalLinks.EventData) => {
...
function handleOther(eventData: UniversalLinks.EventData) {
  ...
  if (eventData.path.search('^/([a-zA-Z0-9]{8})$') !== -1) {
    getChallenge(pieces[1])
      .then(() => router.set('/challenge/' + pieces[1]))
      .catch(() => router.set('/game/' + pieces[1]))
  } else if (
    eventData.path.search('^/([a-zA-Z0-9]{8})+/+(white|black)$') !== -1
  ) {
