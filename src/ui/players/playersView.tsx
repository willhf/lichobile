...
export function searchModal(ctrl: State) {
  if (!ctrl.isSearchOpen()) return null

  const className = ['modal', 'show', settings.general.theme.background()].join(
    ' '
  )
  ...
  return (
    <div id="searchPlayersModal" className={className}>
      <header class="main_header">
        <button
          className="main_header_button"
          oncreate={helper.ontap(() => ctrl.closeSearch())}
        >
          {backArrow}
        </button>
        ...
        {ctrl.searchResults().map(u => {
          return (
            <li
              ...
              key={u}
              oncreate={helper.ontapY(() => ctrl.goToProfile(u))}
            >
              {u}
            </li>
          )
...
export function body(ctrl: State) {
  return (
    <ul
      className="playersSuggestion native_scroller page"
...
function renderPlayer(user: User) {
  const perf = Object.keys(user.perfs).reduce((prev, curr) => {
    ...
    if (user.perfs[prev].rating < user.perfs[curr].rating) return curr
    else return prev
  }) as PerfKey
  return (
