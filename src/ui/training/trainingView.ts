    ...
    : header(
        h('div.main_header_title.withSub', [
          ...
          h(
            ...
            ([
              i18n('rating'),
              ' ' + (ctrl.vm.mode === 'view' ? ctrl.data.puzzle.rating : '?'),
              ' • ',
              i18n('playedXTimes', ctrl.data.puzzle.attempts)
            ] as Mithril.Child[]).concat(
              !ctrl.data.online && !hasNetwork()
                ? [
                    ' • ',
...
export function renderContent(ctrl: TrainingCtrl, key: string, bounds: Bounds) {
  ...
  return h.fragment({ key }, [
    ...
    h('div.training-tableWrapper', [
      h(
        'div.training-table.native_scroller',
...
export function overlay(ctrl: TrainingCtrl) {
  return [renderPromotion(ctrl), menu.view(ctrl.menu)]
}
...
function renderActionsBar(ctrl: TrainingCtrl) {
  return h('section#training_actions.actions_bar', [
    ...
    h('button.action_bar_button.training_action.fa.fa-share-alt', {
      key: 'sharePuzzle',
      oncreate: helper.ontap(ctrl.share, () =>
        window.plugins.toast.show('Share this puzzle', 'short', 'bottom')
      )
    }),
...
function renderViewControls(ctrl: TrainingCtrl) {
  return [
    h(
      'div.li-button.training-control.retry',
      {
    ...
    h(
      'div.li-button.training-control.continue',
      {
...
function renderViewSolution(ctrl: TrainingCtrl) {
  return ctrl.vm.canViewSolution
    ? h(
        'button.fatButton',
        {
...
function renderResult(ctrl: TrainingCtrl) {
  if (ctrl.vm.lastFeedback === 'win') {
    ...
    return [
      h('div.training-half', [h('strong', 'Puzzle complete!')]),
      h('div.training-half', renderViewControls(ctrl))
    ]
  }
}
