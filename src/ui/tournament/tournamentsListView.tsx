  ...
  return (
    <div className="actions_bar">
      <button
        ...
        className="action_create_button"
        oncreate={helper.ontap(newTournamentForm.open)}
      >
...
export function renderTournamentList(list: Array<TournamentListItem>) {
  return h(
    'ul.native_scroller.tournamentList',
    {
...
function renderTournamentListItem(
  ...
  return (
    <li
      key={tournament.id}
      className={
        'list_item tournament_item' +
        evenOrOdd +
        (tournament.createdBy === 'lichess' ? ' official' : '')
      }
    ...
    >
      <div className="tournamentListName">
        ...
        <small className="infos">
          {time} {variant} {mode} • {duration}
        </small>
      </div>
      <div className="tournamentListTime">
        <div className="time">
          {formatTime(tournament.startsAt)}{' '}
          <strong className="timeArrow">-</strong>{' '}
          {formatTime(tournament.finishesAt)}
        </div>
        <small className="nbUsers withIcon" data-icon="r">
