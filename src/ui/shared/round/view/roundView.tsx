...
function overlay(ctrl: OnlineRound) {
  return [
    ...
    miniUser(
      ...
      ctrl.vm.miniUser.player.data,
      ctrl.vm.miniUser.player.showing,
      () => ctrl.closeUserPopup('player')
    ),
    miniUser(
      ctrl.data.opponent.user,
      ctrl.vm.miniUser.opponent.data,
      ctrl.vm.miniUser.opponent.showing,
      () => ctrl.closeUserPopup('opponent')
    )
  ]
}
...
export function renderMaterial(material: Material) {
  return Object.keys(material).map((role: Role) =>
    h(
      'div.tomb',
      { key: role },
      range(material[role]).map(_ => h('piece', { className: role }))
    )
  )
}
...
function renderTitle(ctrl: OnlineRound) {
  ...
  if (ctrl.vm.offlineWatcher || socket.isConnected()) {
    const isCorres =
      !data.player.spectator && data.game.speed === 'correspondence'
    if (ctrl.data.tv) {
      return h(
          ...
          h(
            ...
            tournament
              ? [
                  h('span.fa.fa-trophy'),
                  h(CountdownTimer, {
                    seconds: tournament.secondsToFinish || 0
                  }),
                  h('span', ' • ' + tournament.name)
                ]
              : [
                  h(
                    `span.withIcon[data-icon=${utils.gameIcon(
                      ctrl.data.game.perf
                    )}]`
                  ),
...
function renderHeader(ctrl: OnlineRound) {
  ...
  if (!ctrl.data.tv && !ctrl.data.userTV && ctrl.data.player.spectator) {
    children = [backButton(renderTitle(ctrl))]
  } else {
    children = [menuButton(), renderTitle(ctrl)]
  }
...
function renderContent(ctrl: OnlineRound, isPortrait: boolean) {
  ...
  const opponent = renderPlayTable(
    ctrl,
    ctrl.data.opponent,
...
function renderExpiration(
  ...
  return h(
    ...
    h(CountdownTimer, {
      ...
      textWrap: (t: string) =>
        `<strong>${t}</strong> seconds to play the first move`,
      showOnlySecs: true
    })
  )
}
...
function getChecksCount(ctrl: OnlineRound, color: Color) {
  const player =
    color === ctrl.data.player.color ? ctrl.data.opponent : ctrl.data.player
  ...
  if (ctrl.vm.moveToSubmit || ctrl.vm.dropToSubmit) {
    return (
      <div className="overlay_popup_wrapper submitMovePopup">
        <div className="overlay_popup">{gameButton.submitMove(ctrl)}</div>
      </div>
    )
  }
...
function userInfos(
  ...
  if (user) {
    ...
    let onGameStatus = player.onGame
      ? 'currently on this game'
      : 'currently not on this game'
    let engine =
      position === 'opponent' && user.engine
        ? i18n('thisPlayerUsesChessComputerAssistance') + '; '
        : ''
    let booster =
      position === 'opponent' && user.booster
        ? i18n('thisPlayerArtificiallyIncreasesTheirRating') + '; '
        : ''
    title = `${playerName}: ${engine}${booster}${onlineStatus}; ${onGameStatus}`
  } else title = playerName
...
function renderAntagonistInfo(
  ...
  const togglePopup = user
    ? () => ctrl.openUserPopup(position, user.id)
    : utils.noop
  const vConf = user
    ? helper.ontap(togglePopup, () =>
        userInfos(user, player, playerName, position)
      )
    : helper.ontap(utils.noop, () =>
        window.plugins.toast.show(playerName, 'short', 'center')
      )
  ...
  const tournamentRank =
    ctrl.data.tournament && ctrl.data.tournament.ranks
      ? '#' + ctrl.data.tournament.ranks[player.color] + ' '
      : null
  ...
  return (
    <div
      className={
        'antagonistInfos' + (isCrazy ? ' crazy' : '') + (isZen ? ' zen' : '')
      }
      oncreate={vConf}
    >
      <h2 className="antagonistUser">
        {user && user.patron ? (
          <span
            className={
              'patron status ' + (player.onGame ? 'ongame' : 'offgame')
            }
            data-icon=""
          />
        ) : (
          <span
            className={
              'fa fa-circle status ' + (player.onGame ? 'ongame' : 'offgame')
            }
          />
        )}
        {tournamentRank}
        ...
        (user.engine || user.booster) ? (
          <span className="warning" data-icon="j" />
        ) : null}
      </h2>
      {!isCrazy ? (
        <div className="ratingAndMaterial">
          ...
          {checksNb !== undefined ? (
            <div className="checkCount">+{checksNb}</div>
          ) : null}
      ...
      {isCrazy && ctrl.clock
        ? renderClock(
            ctrl.clock,
            player.color,
            ctrl.vm.goneBerserk[player.color],
            runningColor
          )
    ...
    <section className={'playTable' + (isCrazy ? ' crazy' : '')}>
      ...
      {!isCrazy && ctrl.clock
        ...
        : !isCrazy && ctrl.correspondenceClock
          ...
          : null}
      {playable &&
      ((myTurn && position === 'player') ||
        (!myTurn && position === 'opponent'))
        ? renderExpiration(ctrl, position, myTurn)
        : null}
    </section>
  )
}
...
function tvChannelSelector(ctrl: OnlineRound) {
  const channels = perfApi.perfTypes
    .filter(e => e[0] !== 'correspondence')
    .map(e => [e[1], e[0]])
...
function renderGameRunningActions(ctrl: OnlineRound) {
  if (ctrl.data.player.spectator) {
    let controls = [
      ...
      ctrl.data.tv && ctrl.data.player.user
        ? gameButton.userTVLink(ctrl.data.player.user)
        : null,
      ctrl.data.tv && ctrl.data.opponent.user
        ? gameButton.userTVLink(ctrl.data.opponent.user)
        : null
    ]
...
function renderGameEndedActions(ctrl: OnlineRound) {
  ...
  const status =
    ...
    (winner
      ? '. ' +
        i18n(
          winner.color === 'white' ? 'whiteIsVictorious' : 'blackIsVictorious'
        ) +
        '.'
      : '')
...
function renderGamePopup(ctrl: OnlineRound) {
  const header = ctrl.data.tv
    ...
    : !gameApi.playable(ctrl.data) ? () => renderStatus(ctrl) : undefined
...
function renderGameActionsBar(ctrl: OnlineRound) {
  const answerRequired =
    ctrl.data.opponent.proposingTakeback ||
  ...
  const gmClass = (ctrl.data.opponent.proposingTakeback
    ...
    .concat(['action_bar_button', answerRequired ? 'glow' : ''])
    .join(' ')
  ...
  const gmButton = gmDataIcon ? (
    <button
      className={gmClass}
      data-icon={gmDataIcon}
      key="gameMenu"
      oncreate={helper.ontap(ctrl.showActions)}
    />
  ) : (
    <button
      className={gmClass}
  ...
  return (
    <section className="actions_bar">
      ...
      {gameApi.playable(ctrl.data)
        ? null
        : gameButton.analysisBoardIconOnly(ctrl)}
