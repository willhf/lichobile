...
export function renderQuickSetup(onCustom: () => void) {
  return h(
    ...
    xhr.cachedPools.length
      ? xhr.cachedPools.map(p => renderPool(p)).concat(
          h(
            'div.newGame-pool',
            {
              key: 'pool-custom',
...
function renderPool(p: Pool) {
  return h(
    ...
    [h('div.newGame-clock', p.id), h('div.newGame-perf', p.perf)]
  )
}
...
function renderCustomSetup(
  ...
  if (settingsObj.mode() === '1') {
    ...
    colors = [['randomColor', 'random'], ['white', 'white'], ['black', 'black']]
  }

  const modes =
    session.isConnected() && timeMode !== '0'
      ? [['casual', '0'], ['rated', '1']]
      : [['casual', '0']]
    ...
    generalFieldset.push(
      h(
        ...
        [
          ...
          h(
            'div.select_input.inline',
            formWidgets.renderSelect(
              'Min',
              formName + 'rating_min',
              humanSetup.availableRatingRanges.min,
              settingsObj.ratingMin,
              false
            )
          ),
          h(
            ...
            formWidgets.renderSelect(
              'Max',
              formName + 'rating_max',
              humanSetup.availableRatingRanges.max,
              settingsObj.ratingMax,
              false
            )
          )
        ]
    ...
    timeFieldset.push(
      h(
        ...
        formWidgets.renderSelect(
          'time',
          formName + 'time',
          settings.gameSetup.availableTimes,
          settingsObj.time,
          false
        )
      ),
    ...
    timeFieldset.push(
      h(
        ...
        formWidgets.renderSelect(
          'daysPerTurn',
          formName + 'days',
          settings.gameSetup.availableDays.map(utils.tupleOf),
          settingsObj.days!,
          false
        )
      )
