    ...
    return {
      ...
      flip() {
        ...
        root.chessground.set({
          orientation: s.flip
            ? oppositeColor(root.orientation)
            : root.orientation
        })
      },
      ...
      ? h(
          ...
          [
            formWidgets.renderCheckbox(
              i18n('enableLocalComputerEvaluation'),
              'allowCeval',
              settings.analyse.enableCeval,
              v => {
                ctrl.ceval.toggle()
                if (v) ctrl.initCeval()
                else {
    ...
    h(
      ...
      [
        formWidgets.renderCheckbox(
          i18n('showBestMove'),
          'showBestMove',
          settings.analyse.showBestMove,
          ctrl.settings.toggleBestMove
        )
      ]
    ),
    ctrl.source === 'online' &&
    isOnlineAnalyseData(ctrl.data) &&
    gameApi.analysable(ctrl.data)
      ? h(
          'div.action',
          {
            key: 'showComments'
          ...
          [
            formWidgets.renderCheckbox(
              i18n('keyShowOrHideComments'),
              'showComments',
              settings.analyse.showComments,
      ...
      ? h(
          ...
          [
            formWidgets.renderCheckbox(
              'Infinite analysis',
              'ceval.infinite',
              settings.analyse.cevalInfinite,
              ctrl.settings.cevalToggleInfinite
            ),
            cordova.platformId === 'android'
              ? h(
                  'small.caution',
                  'It will stop after 10 minutes when in background'
                )
              : null
          ]
        )
      ...
      ? h(
          ...
          [
            formWidgets.renderSlider(
              ...
              5,
              1,
              settings.analyse.cevalMultiPvs,
              ctrl.settings.cevalSetMultiPv
            )
          ]
        )
      : null,
    ctrl.ceval.allowed && cores > 1
      ? h(
          'div.action',
          {
            key: 'cevalCores'
