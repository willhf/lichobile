...
export default function ExplorerCtrl(root: AnalyseCtrl): IExplorerCtrl {
  ...
  const withGames = !!(
    isSynthetic(root.data) ||
    gameApi.replayable(root.data) ||
    root.data.game.offline
  )
  const effectiveVariant: VariantKey =
    root.data.game.variant.key === 'fromPosition'
      ? 'standard'
      : root.data.game.variant.key

  const config = explorerConfig.controller(
    root.data.game.variant,
    onConfigClose
  )
  ...
  const fetchOpening = debounce(
    (fen: string): Promise<void> => {
      const conf = {
  ...
  function fetch(fen: string) {
    ...
    if (hasTablebase && withGames && tablebaseRelevant(fen))
  ...
  return {
    ...
    fetchMasterOpening: (function() {
      ...
      return function(fen: string) {
        ...
        return openingXhr('standard', fen, { db: 'masters' }, false).then(
          res => {
            masterCache[fen] = res
            return res
          }
