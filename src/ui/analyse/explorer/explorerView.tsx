...
export default function renderExplorer(ctrl: AnalyseCtrl) {
  ...
  return (
    <div id="explorerTable" className={className} key="explorer">
      ...
      {loading ? (
        <div key="loader" className="spinner_overlay">
      ...
      {configOpened || (data && data.opening) ? (
        <span
          key={configOpened ? 'config-onpen' : 'config-close'}
          className="toconf"
...
export function getTitle(ctrl: AnalyseCtrl): Mithril.Children {
  ...
  if (
    ...
    ctrl.data.game.variant.key === 'fromPosition'
  ) {
    if (data && data.tablebase) return 'Endgame tablebase'
    else return 'Opening explorer'
  } else {
    const what =
      data && data.tablebase ? ' endgame tablebase' : ' opening explorer'
...
function showDtz(stm: string, move: Move) {
  if (move.checkmate) return h('result.' + winner(stm, move), 'Checkmate')
    ...
    return h('result.' + winner(stm, move), 'Variant loss')
  else if (move.variant_loss)
    return h('result.' + winner(stm, move), 'Variant win')
  else if (move.insufficient_material)
    return h('result.draws', 'Insufficient material')
    ...
    return h(
      ...
      {
        title:
          'Next capture or pawn move in ' +
          Math.abs(move.dtz) +
          ' half-moves (Distance To Zeroing of the 50 move counter)'
      },
...
function showGameEnd(title: string) {
  return h(
    'div.explorer-data.empty',
    {
      key: 'explorer-game-end' + title
    },
    [h('div.message', [h('h3', [h('i.withIcon[data-icon=]'), title])])]
  )
}
...
function show(ctrl: AnalyseCtrl) {
  ...
  if (data && data.opening) {
    ...
    if (moves.length) {
      return (
        <div key="explorer-tablebase" className="explorer-data">
          {showTablebase(
            ctrl,
            'Winning',
            moves.filter((move: Move) => move.wdl === -2),
            data.fen
          )}
          {showTablebase(
            ctrl,
            'Unknown',
            moves.filter((move: Move) => move.wdl === null),
            data.fen
          )}
          {showTablebase(
            ctrl,
            'Win prevented by 50-move rule',
            moves.filter((move: Move) => move.wdl === -1),
            data.fen
          )}
          {showTablebase(
            ctrl,
            'Drawn',
            moves.filter((move: Move) => move.wdl === 0),
            data.fen
          )}
          {showTablebase(
            ctrl,
            'Loss saved by 50-move rule',
            moves.filter((move: Move) => move.wdl === 1),
            data.fen
          )}
          {showTablebase(
      ...
      return showGameEnd('Variant end')
...
function failing() {
  return h(
    ...
    h('div.failing.message', [
      h('h3', [h('i.withIcon[data-icon=,]'), 'Oops, sorry!']),
