...
export default function renderGameAnalysis(
  ctrl: AnalyseCtrl
): Mithril.BaseNode {
  ...
  return h(
    'div.analyse-gameAnalysis.native_scroller',
    d.analysis
      ? renderAnalysis(ctrl, vd, isPortrait)
      : renderAnalysisRequest(ctrl),
    d.game.moveCentis
      ? renderMoveTimes(ctrl, d.game.moveCentis, vd, isPortrait)
      : null
  )
}
  ...
  return h(
    ...
    [
      ...
      ctrl.analysisProgress
        ? h(
            'div.analyse-gameAnalysis_chartPlaceholder',
        ...
        : h('svg#acpl-chart.analyse-chart', {
            ...
            oncreate({ dom }: Mithril.DOMNode) {
              setTimeout(() => {
                this.updateCurPly = drawAcplChart(
                  dom as SVGElement,
                  ctrl.data,
                  ctrl.node.ply
                )
...
const AcplSummary: Mithril.Component<
  { d: AnalyseData; analysis: RemoteEvalSummary },
  {}
> = {
        ...
        return h('table', [
          ...
          h('tbody', [
            advices.map(a => {
              ...
              return h('tr', [h('th', nb), h('td', i18n(a[1]))])
            }),
            h('tr', [
  ...
  return h(
    ...
    [
      ...
      h('svg#moveTimes-chart.analyse-chart', {
        ...
        oncreate({ dom }: Mithril.DOMNode) {
          setTimeout(() => {
            this.updateCurPly = drawMoveTimesChart(
              dom as SVGElement,
              ctrl.data,
              moveCentis,
              ctrl.node.ply
