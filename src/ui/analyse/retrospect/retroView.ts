...
export default function retroView(
  root: AnalyseCtrl
): Mithril.BaseNode | undefined {
  const ctrl = root.retro
  if (!ctrl) return
  const fb = ctrl.vm.feedback
  return h(
    'div.analyse-retro_box',
    {
...
function jumpToNext(ctrl: IRetroCtrl) {
  return h(
    'div.li-button.retro-half.retro-continue',
    {
      oncreate: helper.ontap(ctrl.jumpToNext)
    },
    [h('i[data-icon=G]'), 'Next']
  )
}
    ...
    h('div', {
      style: {
        width: `${
          node.ceval
            ? 100 *
                Math.max(0, node.ceval.depth - minDepth) /
                (maxDepth - minDepth) +
              '%'
            : 0
        }`
