...
export function renderBody(ctrl: IRelationCtrl) {
  ...
  if (!rel) {
    return (
      <div className="followingListEmpty">{spinner.getVdom('monochrome')}</div>
    )
  } else if (rel.length) {
    ...
    return <div className="followingListEmpty">Oops! Nothing here.</div>
  }
}
    ...
    <li className="list_item followingList">
      <div className="followingPlayerTitle" oncreate={userLink}>
        <div className="user">
          ...
          {obj.title ? (
            <span className="userTitle">{obj.title}&nbsp;</span>
          ) : null}
          {obj.user}
        </div>
        {perfKey ? (
      ...
      <button
        className="followingPlayerItem followingPlayerAction withIcon"
        data-icon="U"
