...
export function renderBody(ctrl: State) {
  ...
  const mins =
    Math.floor(data.stat.count.seconds / 60) - days * 24 * 60 - hours * 60
  const now = Date.now()
  ...
  return (
    <div class="variantPerfBody native_scroller page">
        ...
        <div>
          <svg
            ...
            oncreate={({ dom }: Mithril.DOMNode) =>
              delayDrawChart(graphData, dom as SVGElement)
            }
      ...
      <table class="variantPerf">
        <tbody>
          <tr>
            <th class="variantPerfHeading" colspan="3">
              {' '}
              Current Rating{' '}
            </th>
          </tr>
          <tr>
            ...
            <td class="variantPerf">
              {' '}
              {data.perf.glicko.rating +
                (data.perf.glicko.provisional ? '?' : '')}{' '}
            </td>
            <td> </td>
          </tr>
          ...
          <tr>
            {' '}
            <td class="variantPerfSpacer" colspan="3">
              {' '}
            </td>{' '}
          </tr>
          <tr>
            <th class="variantPerfHeading" colspan="3">
              {' '}
              Other {i18n('rating')} Statistics{' '}
            </th>
          </tr>
          <tr>
            <th class="variantPerf">
              {' '}
              Avg opponent {i18n('rating').toLowerCase()}{' '}
            </th>
          ...
          <tr class={isEmpty(data.stat.highest)}>
            <th class="variantPerf">
              {' '}
              Highest {i18n('rating').toLowerCase()}{' '}
            </th>
            <td class="variantPerf">
              {' '}
              <span class="progress positive">
                {' '}
                {data.stat.highest ? data.stat.highest.int : ''}{' '}
              </span>{' '}
            </td>
            <td class="variantPerf">
              {' '}
              <span class="progress positive">
                {' '}
                {data.stat.highest
                  ? data.stat.highest.at.substring(0, 10)
                  : ''}{' '}
              </span>{' '}
            </td>
          </tr>
          <tr class={isEmpty(data.stat.lowest)}>
            ...
            <td class="variantPerf">
              ...
              <span class="progress positive">
                {' '}
                {data.stat.lowest ? data.stat.lowest.int : ''}{' '}
              </span>{' '}
            </td>
            <td class="variantPerf">
              {' '}
              <span class="progress positive">
                {' '}
                {data.stat.lowest
                  ? data.stat.lowest.at.substring(0, 10)
                  : ''}{' '}
              </span>{' '}
            </td>
          </tr>
          <tr>
            <th class="variantPerf">
              {' '}
              {i18n('tpTimeSpentPlaying', 0).split(':')[0]}{' '}
            </th>
            <td class="variantPerf" colspan="2">
              {' '}
          ...
          <tr>
            ...
            <td class="variantPerf">
              ...
              <span class="progress positive">
                {' '}
                {Math.round(data.stat.count.win / data.stat.count.all * 100) +
                  '%'}{' '}
              </span>{' '}
            </td>
          </tr>
          <tr>
            <th class="variantPerf">
              {' '}
              {toTitleCase(i18n('nbDraws', 0).split(' ')[1])}{' '}
            </th>
            <td class="variantPerf"> {data.stat.count.draw} </td>
            <td class="variantPerf">
              {' '}
              {Math.round(data.stat.count.draw / data.stat.count.all * 100) +
                '%'}{' '}
            </td>
          </tr>
          <tr>
            <th class="variantPerf"> {i18n('losses')} </th>
            <td class="variantPerf sumLine">
              {' '}
              <span class="progress negative">
                {' '}
                {data.stat.count.loss}{' '}
              </span>{' '}
            </td>
            <td class="variantPerf">
              {' '}
              <span class="progress negative">
                {' '}
                {Math.round(data.stat.count.loss / data.stat.count.all * 100) +
                  '%'}{' '}
              </span>{' '}
            </td>
          </tr>
          ...
          <tr>
            ...
            <td class="variantPerfMemo">
              {' '}
              {Math.round(data.stat.count.rated / data.stat.count.all * 100) +
                '%'}{' '}
            </td>
          </tr>
          <tr>
            ...
            <td class="variantPerfMemo">
              {' '}
              {Math.round(data.stat.count.tour / data.stat.count.all * 100) +
                '%'}{' '}
            </td>
          </tr>
          <tr>
            ...
            <td class="variantPerfMemo">
              {' '}
              {Math.round(data.stat.count.berserk / data.stat.count.all * 100) +
                '%'}{' '}
            </td>
          </tr>
          <tr>
            ...
            <td class="variantPerfMemo">
              {' '}
              {Math.round(
                data.stat.count.disconnects / data.stat.count.all * 100
              ) + '%'}{' '}
            </td>
          </tr>
          <tr>
          ...
          <tr class={isEmpty(data.stat.playStreak)}>
            ...
            <td class="variantPerf">
              {' '}
              {data.stat.playStreak.lastDate &&
                data.stat.playStreak.lastDate.substring(0, 10)}{' '}
            </td>
          </tr>
          <tr class={isEmpty(data.stat.resultStreak.win.max.v)}>
            ...
            <td class="variantPerf">
              ...
              <span class="progress positive">
                {' '}
                {data.stat.resultStreak.win.max.v
                  ? data.stat.resultStreak.win.max.v
                  : ''}{' '}
              </span>{' '}
            </td>
            <td class="variantPerf">
              ...
              <span class="progress positive">
                {' '}
                {data.stat.resultStreak.win.max.to
                  ? data.stat.resultStreak.win.max.to.at.substring(0, 10)
                  : ''}{' '}
              </span>{' '}
            </td>
          </tr>
          <tr class={isEmpty(data.stat.resultStreak.loss.max.v)}>
            ...
            <td class="variantPerf">
              ...
              <span class="progress negative">
                {' '}
                {data.stat.resultStreak.loss.max.v
                  ? data.stat.resultStreak.loss.max.v
                  : ''}{' '}
              </span>{' '}
            </td>
            <td class="variantPerf">
              ...
              <span class="progress negative">
                {' '}
                {data.stat.resultStreak.loss.max.to
                  ? data.stat.resultStreak.loss.max.to.at.substring(0, 10)
                  : ''}{' '}
              </span>{' '}
            </td>
          </tr>
      ...
      <div
        class={
          'variantPerfGames noPadding ' +
          isEmpty(data.stat.bestWins.results.length)
        }
      >
...
function renderGame(game: any) {
  const opp =
    (game.opId.title === null ? '' : game.opId.title) + ' ' + game.opId.name
  ...
  return (
    <div
      class="list_item nav"
      oncreate={helper.ontapY(() => router.set('/analyse/online/' + gameId))}
    >
      {opp} ({date})
    </div>
  )
...
function drawChart(graphData: GraphData, el: SVGElement) {
  if (el) {
    ...
    const scaleY = scaleLinear()
      .domain([
        Math.min.apply(null, values) - 50,
        Math.max.apply(null, values) + 50
      ])
      .rangeRound([height, 0])
    ...
    const xAxis = axisBottom(scaleX).tickFormat(multiFormat)
